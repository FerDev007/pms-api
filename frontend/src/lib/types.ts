export interface User { id: number; username: string; nombre: string; activo: boolean; creado_en: string }
...
export interface Supply { id: number; nombre: string; sku: string; upc: string; stock: number; stock_minimo: number; tipo_suministro: string; capacidad_paginas: number; productos_compatibles: string; picture_url: string; impresora_id: number }
