-- Catalog seed, moved out of the FastAPI lifespan (app/db/seed.py) so it survives the
-- retirement of the Python service. Content is seed.sql plus the SITES list that used
-- to live as a hardcoded Python list.
--
-- Printers are looked up by `nombre` rather than by generated id: the old Python seeder
-- relied on identity assigning 1..7 in insert order, which is true but fragile.
...
-- AltaLink C8130/35/45/55/70, C8230/35/45/55/70
...
-- AltaLink B8145, B8155, B8170, B8245, B8255, B8270
...
-- Physical printers on site, formerly the SITES list in app/db/seed.py.
insert into pms_impresora_en_sitio (ip, a_color, nombre, impresora_id)
select s.ip, s.a_color, s.nombre, p.id
from (values
    ('10.250.36.170', false, 'RRHH',                     'VersaLink B7125, B7130, B7135'),
    ('10.250.37.56',  false, 'Oficina del pueblo',       'VersaLink B7125, B7130, B7135'),
    ('10.250.36.197', false, 'PC&L Almacén',             'VersaLink B7125, B7130, B7135'),
    ('10.250.37.32',  false, 'Automáticas',              'VersaLink B7125, B7130, B7135'),
    ('10.250.36.39',  false, 'Jaula de Corte',           'VersaLink B7125, B7130, B7135'),
    ('10.250.36.103', false, 'Jaula MCC',                'VersaLink B7125, B7130, B7135'),
    ('10.250.36.114', false, 'Moldeo',                   'VersaLink B7125, B7130, B7135'),
    ('10.250.36.141', false, 'Finanzas',                 'VersaLink B7125, B7130, B7135'),
    ('10.250.36.123', false, 'Jaula Fakra',              'VersaLink B7125, B7130, B7135'),
    ('10.136.68.4',   false, 'Almacén HN03',             'VersaLink B7125, B7130, B7135'),
    ('10.250.36.177', false, 'Ingeniería',               'VersaLink B7025, B7030, B7035'),
    ('10.250.36.87',  true,  'Colores customer service', 'AltaLink C8130/35/45/55/70, C8230/35/45/55/70'),
    ('10.250.36.195', false, 'TPA Almacén',              'AltaLink B8145, B8155, B8170, B8245, B8255, B8270'),
    ('10.136.67.80',  false, 'Producción HN03',          'AltaLink B8145, B8155, B8170, B8245, B8255, B8270'),
    ('10.250.36.14',  true,  'Colores PC&L',             'VersaLink C410, C415')
) as s (ip, a_color, nombre, modelo)
join pms_impresora p on p.nombre = s.modelo;
